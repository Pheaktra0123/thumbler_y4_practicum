...
<body>
    <main>
        <section class="w-full mb-10">
            <h1 class="text-4xl text-gray-800 uppercase text-center font-medium ">Category Tumbler</h1>
            ...
            <div class="w-11/12 mx-auto flex justify-between item-center place-items-center mt-5">
                ...
                <button data-modal-target="crud-modal" data-modal-toggle="crud-modal" class="flex bg-blue-700 px-5 py-3 rounded-lg" type="button">
                    <p class="text-sm text-white">Add</p>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 w-6 h-6 text-white">
            ...
            <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-md max-h-full">
                    ...
                    <div class="relative bg-white rounded-lg shadow-sm">
                        ...
                        <!-- Modal body -->
                        <form class="p-4 md:p-5" action="{{ route('Categories.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <button type="submit" id="submitBtn" class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                <!-- Normal State -->
                                <div class="flex items-center transition-all opacity-100" id="buttonContent">
                                ...
                                <!-- Loading State -->
                                <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-0 invisible" id="loadingSpinner">
        ...
        <section>
            <div class="flex flex-col shadow-sm rounded-lg w-11/12 mx-auto">
                <div class="overflow-x-auto">
                    <div class="min-w-full inline-block align-middle h-1/2">
                                    ...
                                    <tr class="bg-white transition-all duration-500 hover:bg-gray-50 text-center">
                                        ...
                                        <td class="p-2">
                                            <div class="flex items-center gap-1 justify-center">
                                                ...
                                                <div id="deleteModal-{{$category->id}}" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 hidden">
                                                    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
                                                        ...
                                                        <div class="flex justify-end gap-2">
                                                            <button onclick="hideDeleteModal({{$category->id}})" class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Cancel</button>
                                                            <button onclick="submitDeleteForm({{$category->id}})" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Delete</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div id="edit-modal{{$category->id}}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                                <div class="relative p-4 w-full max-w-md max-h-full">
                                                    ...
                                                    <div class="relative bg-white rounded-lg shadow">
                                                        ...
                                                        <!-- Modal body -->
                                                        <form class="p-4 md:p-5" method="POST" action="/Admin/Categories/Edit/{{$category->id}}" enctype="multipart/form-data">
                                                            @csrf
                                                            @method('PUT')
                                                            ...
                                                            <div class="grid gap-4 mb-4 grid-cols-2">
                                                                <div class="col-span-2">
                                                                    ...
                                                                    <input type="text" name="title" id="name" value="{{$category->name}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Type category name" required>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div class="col-span-2">
                                                                ...
                                                                <div class="w-[370px] relative border-2 border-gray-300 border-dashed rounded-lg p-6" id="dropzone">
                                                                    <input type="file" name="thumbnail" class="absolute inset-0 w-full h-full opacity-0 z-50" id="file-upload">
                                                                    <div class="text-center">
                                                                        <img class="mx-auto h-24 w-24" src="{{ Storage::url($category->Thumbnail) }}" alt="Current Thumbnail">
                                                                        <h3 class="mt-2 text-sm font-medium text-gray-900">
                                                                            <label for="file-upload" class="relative cursor-pointer">
                                                                                <span>Drag and drop</span>
                                                                                <span class="text-indigo-600"> or browse</span>
...
<script>
    ...
    function submitDeleteForm(id) {
        document.querySelector('form[action="/Admin/Categories/Delete/' + id + '"]').submit();
    }
</script>
@endsection