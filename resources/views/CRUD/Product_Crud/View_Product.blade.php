...
<body>
    ...
    <style>
        ...
        .animate-popUp {
            animation: popUp 0.5s cubic-bezier(.36, .07, .19, .97);
        }
                                    ...
                                    <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                                        ...
                                        <td class=" p-5 ">
                                            <div class="flex items-center gap-1">
                                                ...
                                                <button class="p-2 rounded-full group transition-all duration-500 flex item-center" onclick="showDeletePopup({{ $tumbler->id }})" type="button">
    ...
    <div id="delete-popup" class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-40 animate-fadeIn hidden">
        <div class="bg-white rounded-lg shadow-lg p-8 flex flex-col items-center animate-popUp">
            <svg class="w-16 h-16 text-red-500 mb-4 animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <circle cx="12" cy="12" r="10" stroke-width="2" stroke="currentColor" fill="none"/>
                <path stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
            </svg>
            <h2 class="text-2xl font-bold mb-2 text-red-700">Delete Tumbler?</h2>
            <p class="text-gray-700 mb-4" id="delete-popup-message">Are you sure you want to delete this tumbler?</p>
            <div class="flex gap-4">
                <button id="delete-confirm-btn" class="px-6 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition">Delete</button>
                <button onclick="closeDeletePopup()" class="px-6 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400 transition">Cancel</button>
            </div>
        </div>
    </div>

    <div id="delete-success-popup" class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-40 animate-fadeIn hidden">
        <div class="bg-white rounded-lg shadow-lg p-8 flex flex-col items-center animate-popUp">
            <svg class="w-16 h-16 text-green-500 mb-4 animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <circle cx="12" cy="12" r="10" stroke-width="2" stroke="currentColor" fill="none"/>
                <path stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2l4-4"/>
            </svg>
            <h2 class="text-2xl font-bold mb-2 text-green-700">Deleted!</h2>
            <p class="text-gray-700 mb-4">Tumbler deleted successfully.</p>
            <button onclick="closeDeleteSuccessPopup()" class="mt-2 px-6 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">OK</button>
        </div>
    </div>
</body>
...
<script>
    ...
    window.addEditColor = function(id) {
        ...
        let currentColors = [];
        try {
            currentColors = JSON.parse(colorsJson.value);
        } catch {}
    ...
    window.addEditSize = function(id) {
        ...
        let currentSizes = [];
        try {
            currentSizes = JSON.parse(sizesJson.value);
        } catch {}
    ...
    function closeDeletePopup() {
        ...
        deleteTumblerId = null;
    }
    function closeDeleteSuccessPopup() {
        document.getElementById('delete-success-popup').classList.add('hidden');
        // Optionally reload page or remove row from DOM
        location.reload();
    }

    document.getElementById('delete-confirm-btn').onclick = function() {
        if (!deleteTumblerId) return;
        // Corrected AJAX delete request URL
        fetch("{{ url('/Admin/delete/tumbler') }}/" + deleteTumblerId, {
            method: 'DELETE',
            headers: {
        ...
        .then(res => {
            if (res.ok) {
                document.getElementById('delete-popup').classList.add('hidden');
                document.getElementById('delete-success-popup').classList.remove('hidden');
            } else {
                alert('Failed to delete tumbler.');
                closeDeletePopup();
            }
        })
        .catch(() => {
